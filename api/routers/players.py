...
def _player_exists(player_id: str) -> bool:
    if not _gold_table_exists("master_players"):
        return False
    df = duckdb_client.execute(
        f"SELECT 1 FROM master_players WHERE player_id = '{player_id}' LIMIT 1"
    )
    return not df.empty
...
def get_player_full_profile(player_id: str):
    ...
    identity_df = duckdb_client.execute(
        f"SELECT * FROM master_players WHERE player_id = '{player_id}' LIMIT 1"
    )
    ...
    if _gold_table_exists("player_athletic_profiles"):
        ath_df = duckdb_client.execute(
            f"SELECT * FROM player_athletic_profiles WHERE player_id = '{player_id}' LIMIT 1"
        )
        result["athletic"] = (
    ...
    if _gold_table_exists("player_durability_profiles"):
        dur_df = duckdb_client.execute(
            f"SELECT * FROM player_durability_profiles WHERE player_id = '{player_id}' LIMIT 1"
        )
        result["durability"] = (
